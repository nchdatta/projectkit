...
| Styling         | Tailwind CSS 4                              | CSS-first config in `src/app/globals.css`. There is no `tailwind.config.js` and you should not create one |
| Data            | Prisma 7 + PostgreSQL 18 (local)            | Generated client is in `src/generated/prisma` (gitignored)                                                |
...
src/hooks/queries/<resource>.ts            TanStack Query hooks, keyed from keys.ts
...
- **Prisma is imported only from `src/lib/db.ts`.** Never `new PrismaClient()` anywhere else, never import from `src/generated/prisma` outside `db.ts`.
...
`src/hooks/queries/health.ts`, and the two specs in `src/services/__tests__/`.
...
- The client is generated into `src/generated/prisma`, not `node_modules`. After any schema edit: `npm run db:generate`.
