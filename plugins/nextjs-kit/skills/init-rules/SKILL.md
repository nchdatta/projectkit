...
The agents and commands in this plugin reference `.claude/rules/*.md` by path. Rules only auto-load from a project's own `.claude/rules/`, so they must be copied in — the plugin ships them as payload rather than loading them itself.

## 1. Find the plugin root

You need the directory containing this plugin's `rules/` and `bin/`. Locate it by globbing for a `nextjs-kit` directory that contains `bin/init-rules.mjs`, checking, in order:

- the path passed to `--plugin-dir`, if this session was started that way;
- the plugin cache under the user's home directory (`~/.claude/plugins/**/nextjs-kit*/bin/init-rules.mjs`).
...
- **Stack mismatch.** The rules assume Next.js App Router, Prisma, TanStack Query, zod, and Vitest + MSW. Name anything the project does not use, because those rules will mislead the agents.
- **Layer paths.** The agents guard `src/services/**`, `prisma/**`, `src/app/api/**` and so on. If the project's layout differs, the scope lines need editing.
