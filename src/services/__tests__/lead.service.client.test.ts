...
// Reads go through `leadService` (fetcher.ts) — see lead.service.test.ts; this file covers writes only.
describe("leadClientService", () => {
  it("posts a new lead and returns it", async () => {
    ...
    await expect(
      leadClientService.createLead({ name: lead.name, email: lead.email }),
    ).resolves.toEqual(lead);
  });

  it("normalizes a failure envelope into an ApiError", async () => {
    server.use(
      mswHttp.patch(`${API_URL}/leads/${lead.id}`, () =>
        HttpResponse.json(
    ...
    await expect(
      leadClientService.updateLead(lead.id, { name: "New name" }),
    ).rejects.toBeInstanceOf(ApiError);
    await expect(leadClientService.updateLead(lead.id, { name: "New name" })).rejects.toMatchObject(
      {
  ...
  it("exposes validation problems keyed by field", async () => {
    ...
    const error = await leadClientService
      .createLead({ name: lead.name, email: "not-an-email" })
